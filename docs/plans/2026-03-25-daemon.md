...
class Daemon:
    ...
    Run on a machine like your GPU rig that should always be working.
    The daemon:
...
# Launch the HotBunk daemon in a tmux session on the always-on box
#
# Usage: ssh always-on-box 'bash -s' < deploy/launch-submarine.sh
...
git add deploy/
git commit -m "Add systemd service and tmux launcher for the always-on box"
```
